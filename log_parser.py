...
with open('mule.log', 'r') as hosts:
    for line in hosts:
        ...
        if "one_slot INFO Fast block at Node 0 for Epoch" in line:
            ...
            diff_time = datetime.datetime.strptime(params[1], "%H:%M:%S,%f") - start
            tx_cnt += int(params[-1])
            print("fast-block", '\t', diff_time.total_seconds(), '\t', params[-2].replace(",", ""), '\t', tx_cnt)
            pre_time = diff_time
        ...
        if "INFO Node 0 Delivers ACS Block in Epoch" in line:
            ...
            diff_time = datetime.datetime.strptime(params[1], "%H:%M:%S,%f") - start
            tx_cnt += int(params[-1])
            print("acs-block", '\t', diff_time.total_seconds(), '\t', (diff_time - pre_vc_time).total_seconds(), '\t', tx_cnt)
            pre_time = diff_time
...
with open('dumbo.log', 'r') as hosts:
    for line in hosts:
        ...
        if "INFO Node 0 Delivers ACS Block in Round" in line:
            ...
            diff_time = datetime.datetime.strptime(params[1], "%H:%M:%S,%f") - start
            tx_cnt += int(params[-2])
            print("acs-block", '\t', diff_time.total_seconds(), '\t', diff_time.total_seconds() - pre_time.total_seconds(), '\t', tx_cnt)
            pre_time = diff_time
print()
